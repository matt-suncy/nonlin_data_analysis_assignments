...
% Starter coder for CISC371, Fall 2021, Assignment #4, Question #2
    ...
    % %
    % Constraint value for CLS regression
    theta = 8;
    ...
    % Adjust x and y axes to 0.5 beyond the data
    ...
    % Add a legend
    legend('Data', 'OLS', 'CLS', 'Location', 'SouthEast');

    tstring = strcat('\bf{}CLS fit: $\boldmath{}\|\vec{w}\|^2 = ', ...
        sprintf('%0.2f', norm(w_cls)^2), '$');
    % Also include the squared norm of the OLS solution
    tstring_2 = strcat('\bf{}OLS fit: $\boldmath{}\|\vec{w}\|^2 = ', ...
        sprintf('%0.2f', norm(w_ols)^2), '$');
    % Add the title
    title({tstring, tstring_2}, 'Interpreter', 'latex', 'FontSize', 14);
    ...
    % %
    % Mean is the mean of the errors
    % Used std() to find standard deviation of the errors
    olsTrainMean   = sum(olsTrainVec) / nreps;
    olsTrainStdDev = std(olsTrainVec);
    clsTrainMean   = sum(clsTrainVec) / nreps;
    clsTrainStdDev = std(clsTrainVec);
    olsTestMean    = sum(olsTestVec) / nreps;
    olsTestStdDev  = std(olsTestVec);
    clsTestMean    = sum(clsTestVec) / nreps;
...
%   
    % lval is the input Lagrange multiplier
    wfun = @(lval) inv(xmat'*xmat + lval*Im) * xmat' * yvec;
    % Interim function g used to find lambda by finding root of g
...
% %        
        % Use fzero() to find root of gfun
        % Initial guess for lambda is 0 since that is when CLS solution is equal to OLS
        lambda = fzero(gfun, 0);
        % Use lambda to find CLS solution
        w_cls = wfun(lambda);
    end
end
